...
/** MyBatis adapter for the system directory. It is the only EIP class aware of sys_* tables. */
@Component
public class RecipientDirectoryAdapter implements RecipientDirectory {
    ...
    public RecipientDirectoryAdapter(UserMapper userMapper, RoleInfoMapper roleMapper,
                                     PermissionMapper permissionMapper, DeptMapper deptMapper,
    ...
    public Set<Long> resolve(RecipientSelectorDTO selector) {
        ...
        if (selector.isAllActiveUsers()) {
            candidates.addAll(userMapper.selectList(new LambdaQueryWrapper<User>()
                    .eq(User::getStatus, UserStatus.NORMAL)).stream().map(User::getId).toList());
        }
        ...
        if (selector.getPermissionCodes() != null && !selector.getPermissionCodes().isEmpty()) {
            Set<Long> permissionIds = permissionMapper.selectList(new LambdaQueryWrapper<Permission>()
                    .in(Permission::getCode, selector.getPermissionCodes())
                    .eq(Permission::getStatus, Status.ENABLE)).stream().map(Permission::getId).collect(Collectors.toSet());
            if (!permissionIds.isEmpty()) {
                Set<Long> roleIds = rolePermissionMapper.selectList(new LambdaQueryWrapper<RolePermission>()
                        .in(RolePermission::getPermissionId, permissionIds)).stream().map(RolePermission::getRoleId)
        ...
        if (selector.getDepartmentIds() != null && !selector.getDepartmentIds().isEmpty()) {
            ...
            candidates.addAll(userMapper.selectList(new LambdaQueryWrapper<User>()
                    .in(User::getDeptId, departmentIds)).stream().map(User::getId).toList());
        }
        // Administrator expansion is opt-in. In particular, USER alone never reaches this branch.
        if (selector.isIncludeAdministrators()) {
            Set<Long> adminRoles = roleMapper.selectList(new LambdaQueryWrapper<RoleInfo>()
                    .and(w -> w.eq(RoleInfo::getCode, "admin").or().eq(RoleInfo::getName, "管理员"))
                    .eq(RoleInfo::getStatus, RoleEnum.ENABLE)).stream().map(RoleInfo::getId).collect(Collectors.toSet());
            candidates.addAll(usersByRoles(adminRoles));
        }
        if (candidates.isEmpty()) return Collections.emptySet();
        return userMapper.selectList(new LambdaQueryWrapper<User>().in(User::getId, candidates)
                        .eq(User::getStatus, UserStatus.NORMAL)).stream().map(User::getId)
                .collect(Collectors.toCollection(LinkedHashSet::new));
    }

    private Set<Long> roleIds(Set<String> codes) {
        return roleMapper.selectList(new LambdaQueryWrapper<RoleInfo>().in(RoleInfo::getCode, codes)
                        .eq(RoleInfo::getStatus, RoleEnum.ENABLE)).stream().map(RoleInfo::getId)
                .collect(Collectors.toSet());
    }
    ...
    private Set<Long> includeDescendants(Set<Long> roots) {
        List<Dept> enabled = deptMapper.selectList(new LambdaQueryWrapper<Dept>().eq(Dept::getStatus, DeptStatus.ENABLE));
        ...
        while (!queue.isEmpty()) {
            ...
            for (Dept dept : enabled) {
                if (parent.equals(dept.getParentId()) && result.add(dept.getId())) queue.addLast(dept.getId());
            }
        }
        return result;
    }
    ...
    public List<RecipientOptionVO> options(RecipientSelectorType type, String keyword) {
        ...
        return switch (type) {
            case USER -> userMapper.selectList(new LambdaQueryWrapper<User>().eq(User::getStatus, UserStatus.NORMAL)
                    .and(!query.isBlank(), w -> w.like(User::getUsername, query).or().like(User::getRealName, query)))
                    .stream().map(u -> new RecipientOptionVO(type, String.valueOf(u.getId()), display(u.getRealName(), u.getUsername()))).toList();
            case ROLE -> roleMapper.selectList(new LambdaQueryWrapper<RoleInfo>().eq(RoleInfo::getStatus, RoleEnum.ENABLE)
                    .and(!query.isBlank(), w -> w.like(RoleInfo::getCode, query).or().like(RoleInfo::getName, query)))
                    .stream().map(r -> new RecipientOptionVO(type, r.getCode(), r.getName())).toList();
            case PERMISSION -> permissionMapper.selectList(new LambdaQueryWrapper<Permission>().eq(Permission::getStatus, Status.ENABLE)
                    .and(!query.isBlank(), w -> w.like(Permission::getCode, query).or().like(Permission::getName, query)))
                    .stream().map(p -> new RecipientOptionVO(type, p.getCode(), p.getName())).toList();
            case DEPARTMENT -> deptMapper.selectList(new LambdaQueryWrapper<Dept>().eq(Dept::getStatus, DeptStatus.ENABLE)
                    .and(!query.isBlank(), w -> w.like(Dept::getCode, query).or().like(Dept::getName, query)))
                    .stream().map(d -> new RecipientOptionVO(type, String.valueOf(d.getId()), d.getName())).toList();
