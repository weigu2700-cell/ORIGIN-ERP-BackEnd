...
public class NotificationTemplateServiceImpl implements NotificationTemplateService {
    ...
    public List<NotificationTemplateDTO> list(String keyword) {
        ...
        return templateMapper.selectList(new LambdaQueryWrapper<NotificationTemplate>()
                        .and(w -> w.like(NotificationTemplate::getCode, value)
                                .or().like(NotificationTemplate::getName, value))
                        .orderByDesc(NotificationTemplate::getUpdateTime)).stream().map(this::toDto).toList();
    }
    ...
    public NotificationTemplateDTO update(Long id, NotificationTemplateDTO dto) {
        if (templateMapper.selectById(id) == null) throw new BusinessException(404, "通知模板不存在");
        normalizeAndValidate(dto);
        NotificationTemplate template = new NotificationTemplate();
        BeanUtils.copyProperties(dto, template);
        template.setId(id);
    ...
    public void updateStatus(Long id, Status status) {
        if (templateMapper.update(null, new LambdaUpdateWrapper<NotificationTemplate>()
                .eq(NotificationTemplate::getId, id).set(NotificationTemplate::getStatus, status)) == 0) {
    ...
    public void delete(Long id) {
        ...
        if (templateMapper.deleteById(id) == 0) throw new BusinessException(404, "通知模板不存在");
    }
    ...
    private void replaceRecipients(Long templateId, List<NotificationTemplateRecipientDTO> values) {
        ...
        for (NotificationTemplateRecipientDTO dto : values) {
            if (dto == null || dto.getSelectorType() == null || dto.getSelectorValue() == null
                    || dto.getSelectorValue().isBlank()) continue;
    ...
    private void normalizeAndValidate(NotificationTemplateDTO dto) {
        ...
        boolean hasPresetRecipient = dto.getRecipients() != null && dto.getRecipients().stream()
                .anyMatch(value -> value != null
                        && value.getSelectorType() != null
                        && value.getSelectorValue() != null
    ...
    private NotificationTemplateDTO toDto(NotificationTemplate template) {
        ...
        dto.setRecipients(recipientMapper.selectList(new LambdaQueryWrapper<NotificationTemplateRecipient>()
                        .eq(NotificationTemplateRecipient::getTemplateId, template.getId())
                        .orderByAsc(NotificationTemplateRecipient::getId)).stream().map(row -> {
