...
class MaterialStockConcurrencyIntegrationTests {
    ...
    private static final MySQLContainer MYSQL = new MySQLContainer("mysql:8.4")
            .withDatabaseName("origin_erp_test")
            .withUsername("origin")
            .withPassword("origin");
    ...
    static void mysqlProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("security.jwt.secret",
                () -> "c21hcnQtZXJwLXRlc3Qtc2VjcmV0LW11c3QtYmUtYXQtbGVhc3QtMzItYnl0ZXM=");
    }
    ...
    void concurrentFirstInboundCreatesOneStockRowAndTwoLedgerRows() throws Exception {
        ...
        try {
            Future<?> first = executor.submit(() -> inboundAfterBarrier(
                    ready, start, new BigDecimal("2"), "PI-CONCURRENT-1"));
            Future<?> second = executor.submit(() -> inboundAfterBarrier(
                    ready, start, new BigDecimal("3"), "PI-CONCURRENT-2"));
        ...
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM inv_material_stock WHERE material_id = ? AND warehouse_id = ?",
                Long.class, MATERIAL_ID, WAREHOUSE_ID)).isEqualTo(1L);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT on_hand FROM inv_material_stock WHERE material_id = ? AND warehouse_id = ?",
                BigDecimal.class, MATERIAL_ID, WAREHOUSE_ID)).isEqualByComparingTo("5");
        assertThat(jdbcTemplate.queryForObject(
                "SELECT version FROM inv_material_stock WHERE material_id = ? AND warehouse_id = ?",
                Integer.class, MATERIAL_ID, WAREHOUSE_ID)).isEqualTo(1);
    ...
    private void inboundAfterBarrier(
