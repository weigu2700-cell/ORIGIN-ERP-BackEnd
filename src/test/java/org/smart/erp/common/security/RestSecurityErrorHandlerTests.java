...
class RestSecurityErrorHandlerTests {
	...
	void authenticationFailureReturnsHttp401Result() throws Exception {
		...
		new RestAuthenticationEntryPoint(objectMapper).commence(new MockHttpServletRequest(), response,
	...
	void filterAuthorizationFailureReturnsHttp403Result() throws Exception {
		...
		new RestAccessDeniedHandler(objectMapper).handle(new MockHttpServletRequest(), response,
